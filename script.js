...
function setupEventListeners() {
    ...
    // Add other listeners as needed, e.g., for dashboard buttons
     ...
     savePracticeSessionBtn.addEventListener('click', async () => {
         clearError('practiceSessionError');
         const selectedInstrument = sessionInstrumentSelect.value;

         if (!selectedInstrument) {
             displayError('practiceSessionError', 'Please select an instrument for the session.');
             return;
         }

         // Filter only submitted subsessions (ensure they have minutes > 0)
         const submittedSubsessions = currentSubsessions.filter(sub => sub.submitted && sub.minutes > 0);

         if (submittedSubsessions.length === 0) {
             displayError('practiceSessionError', 'Please add and submit at least one subsession (with time > 0).');
             return;
         }

         if (!currentUser) {
             displayError('practiceSessionError', 'Error: No logged-in user found. Please sign in again.');
             return;
         }
         ...
         try {
             ...
             console.error('Error in save session button handler:', error);
             displayError('practiceSessionError', `Failed to save practice session: ${error.message}`);
         }
     });
}
...
const savePracticeSession = async (sessionData) => {
    console.log("savePracticeSession received:", sessionData);
    if (!sessionData || !sessionData.subsessions) {
        console.error("savePracticeSession called without valid sessionData or subsessions");
        throw new Error("Internal error: Invalid data provided for saving session.");
    }
    ...
    try {
        ...
        // Calculate total minutes from the *passed* subsessions array
        const totalMinutes = sessionData.subsessions.reduce((total, subsession) => {
            // Ensure we are summing the correct property (e.g., 'duration' or 'minutes')
            return total + (subsession.duration || 0); // Using 'duration' as mapped above
        }, 0);
        console.log("Calculated total minutes for save:", totalMinutes);

        if (totalMinutes <= 0) {
            throw new Error("Practice session has no duration.");
        }

        // Update today's practice time in the 'users' table
        const { error: updateError } = await supabase
            .from('users') // Assuming 'public.users' table
            .update({ 
                minutes_practiced_today: supabase.rpc('increment_user_practice_time', { 
                    user_id_input: user.id, 
                    minutes_input: totalMinutes 
                })
             })
            .eq('id', user.id);
        // Note: Using an RPC might be safer for increments, let's create one.

        if (updateError) {
             console.error("Error updating user practice time:", updateError);
             throw updateError; // Re-throw to be caught below
        }
        console.log("User daily practice time updated.");

        // Prepare data for practice_sessions table
        const sessionToInsert = {
            user_id: user.id,
            instrument: sessionData.instrument,
            total_minutes: totalMinutes,
             // Store the detailed subsessions as JSONB
            subsessions_data: sessionData.subsessions
        };
        console.log("Inserting into practice_sessions:", sessionToInsert);

        // Save the main practice session record
        const { data, error } = await supabase
            .from('practice_sessions') // Assuming 'public.practice_sessions'
            .insert([sessionToInsert]);

        if (error) {
            console.error("Error inserting into practice_sessions:", error);
            throw error; // Re-throw
        }

        console.log("Practice session insert successful:", data);
        return data;
    } catch (error) {
        console.error('Error saving practice session:', error);
        // Rethrow the error so the caller (button handler) can catch it
        throw error;
    }
};
